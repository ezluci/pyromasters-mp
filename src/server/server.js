...
// -------------- RANDOM FUNCTIONS -------------- (i need to move these in separate files)
   ...
   sok.START_GAME = () => {
      ...
      sok.setRoomStatus(ROOM_STATUS.RUNNING);
      ...
      let gameTime_intervalId = setInterval(() => {
         ...
         if (ROOMS.get(sok.room).gameTime === 0) {
            ...
            gameTime_intervalId = setInterval(() => {
               ...
               filled[yg][xg] = true;
               sok.map[yg][xg] = BLOCK.PERMANENT;

               io.to(sok.room).emit('mapUpdates', [{x: xg, y: yg, block: BLOCK.PERMANENT}]);
               ...
               ['white', 'black', 'orange', 'green'].forEach((color) => {
                  if (!ROOMS.get(sok.room)[color].dead && !ROOMS.get(sok.room)[color].shield && sok.onDeadlyBlockCheck(color)) {
                     ...
                     ROOMS.get(sok.room)[color].coords = Object.assign(INEXISTENT_POS);
   
                     if (sok.countNotDead() <= 1) {
                        sok.intervalIDS.add( setTimeout(sok.showEndScreen, END_SCREEN_TIMEOUT) );
                     }
                  }
               });
      ...
      ['white', 'black', 'orange', 'green'].forEach(color => {
         if (ROOMS.get(sok.room)[color].selected === false) {
            ...
            ROOMS.get(sok.room)[color].coords = Object.assign(DEFAULT_POS[color]);
         ...
         for (let x = 0; x < BLOCKS_HORIZONTALLY; ++x) {
            if (y % 2 == 1 && x % 2 == 1) {
               ...
               if (!canDraw)
                  ...
                  sok.map[y][x] = (Math.random() < .7 ? BLOCK.NORMAL : BLOCK.NO); // need to check the original code here!!
            }
         }
      }
