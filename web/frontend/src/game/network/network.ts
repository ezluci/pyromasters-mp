...
import { processPacket_guestCount } from './in-packets/guest-count';
...
function startHandlingPackets(this: Network, g: Game) {
  ...
  // set button actions
  Dom.selectWhite.addEventListener('click', () =>
    sendPacket_selectColor(g, Color.WHITE),
  );
  Dom.selectBlack.addEventListener('click', () =>
    sendPacket_selectColor(g, Color.BLACK),
  );
  Dom.selectOrange.addEventListener('click', () =>
    sendPacket_selectColor(g, Color.ORANGE),
  );
  Dom.selectGreen.addEventListener('click', () =>
    sendPacket_selectColor(g, Color.GREEN),
  );
  Dom.selectSpectator.addEventListener('click', () =>
    sendPacket_selectColor(g, null),
  );
  ...
  const pingLoopIntv = setInterval(() => {
    if (
      ...
      this.server.readyState === WebSocket.CLOSED
    ) {
    ...
    const timeSinceFirstUnresolved =
      (time - this.unresolvedPings[0] + this.timeBitmask + 1) &
      this.timeBitmask;
...
function processFrame(this: Network, frame: Uint8Array) {
  ...
  while (idx < frame.length) {
    ...
    } else if (packetType === Network.char_guestCount) {
      processPacket_guestCount(packet, this.g);
    } else if (packet[0] <= 13) {
