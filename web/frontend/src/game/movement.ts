...
function stop(g: Game, x: number, y: number): boolean {
  ...
  return (
    g.grid[x][y] === Block.NORMAL ||
    g.grid[x][y] === Block.PERMANENT ||
    getBomb(g, x, y) !== null
  );
}
...
function getBomb(g: Game, x: number, y: number): Bomb | null {
  ...
  const filteredBombs = g.bombs.filter(
    (bomb) =>
...
export function moveLeft(g: Game) {
  ...
  if (
    (BLOCK_SIZE - BLOCK_SAFE_PX <= mod && mod <= BLOCK_SIZE + BLOCK_SAFE_PX) ||
    me.x === MIN_X
  ) {
  ...
  if (mod === 0) {
    ...
    if (me.x < MIN_X) {
      ...
      Math.floor(me.x / BLOCK_SIZE) !== Math.floor(xOld / BLOCK_SIZE)
    ) {
      // if we go in a different block
    ...
    if (
      me.x % BLOCK_SIZE === 0 &&
      (tmpBomb = getBombExact(g, me.x / BLOCK_SIZE - 1, me.y / BLOCK_SIZE))
    ) {
    ...
    if (mod < BLOCK_SIZE) {
      // move up
      if (
        (tmpBomb = getBombExact(
          g,
          me.x / BLOCK_SIZE - 1,
          Math.floor(me.y / BLOCK_SIZE),
        ))
      ) {
        Animations.changeAnimation(g.myPlayer, Animation.WALK_LEFT);
        sendPacket_kickBomb(g, tmpBomb, -1, 0);
      } else if (
        !stop(g, me.x / BLOCK_SIZE - 1, Math.floor(me.y / BLOCK_SIZE))
      ) {
      ...
      if (
        (tmpBomb = getBombExact(
          g,
          me.x / BLOCK_SIZE - 1,
          Math.ceil(me.y / BLOCK_SIZE),
        ))
      ) {
        Animations.changeAnimation(g.myPlayer, Animation.WALK_LEFT);
        sendPacket_kickBomb(g, tmpBomb, -1, 0);
      } else if (
        !stop(g, me.x / BLOCK_SIZE - 1, Math.ceil(me.y / BLOCK_SIZE))
      ) {
...
export function moveDown(g: Game) {
  ...
  if (mod === 0) {
    ...
    if (
      me.y % BLOCK_SIZE === 0 &&
      (tmpBomb = getBombExact(g, me.x / BLOCK_SIZE, me.y / BLOCK_SIZE + 1))
    ) {
    ...
    if (mod < BLOCK_SIZE) {
      if (
        ...
        !stop(g, Math.floor(me.x / BLOCK_SIZE), me.y / BLOCK_SIZE + 1)
      ) {
        Animations.changeAnimation(g.myPlayer, Animation.WALK_LEFT);
        me.x -= speed * g.deltaTime;
      ...
      if (
        ...
        !stop(g, Math.ceil(me.x / BLOCK_SIZE), me.y / BLOCK_SIZE + 1)
      ) {
...
export function moveUp(g: Game) {
  ...
  if (mod === 0) {
    ...
    if (me.y < MIN_Y) {
      ...
      Math.floor(me.y / BLOCK_SIZE) !== Math.floor(yOld / BLOCK_SIZE)
    ) {
