...
export function drawPlayerImage(
  this: Game,
  image: HTMLImageElement,
  x: number,
  y: number,
  ...
  // 53x78
  this.ctx.drawImage(image, OFFSET_LEFT + x, OFFSET_UP + y - 25, 53, 78);
}
...
export function drawPlayer(this: Game, player: Player) {
  ...
  const animIdx =
    Math.floor(
      (performance.now() - animation.startTime) * player.speed * 0.42,
    ) % animation.spriteInfos.length;
...
export function drawFrame(this: Game) {
  ...
  // draw background
  this.ctx.drawImage(
    Resources.images.maps[this.map].background,
    OFFSET_LEFT,
    OFFSET_UP,
    Dom.canvas.width - OFFSET_LEFT - OFFSET_RIGHT,
    Dom.canvas.height - OFFSET_UP - OFFSET_DOWN,
  );
  ...
  for (let y = 0; y < BLOCKS_VERTICALLY; ++y)
    for (let x = 0; x < BLOCKS_HORIZONTALLY; ++x) {
      if (
        this.map === Map.FOURWAY &&
        MAP_FOURWAY_PORTAL_POSITIONS.filter(
          ({ x: xx, y: yy }) => xx === x && yy === y,
        ).length === 1
      ) {
      ...
      switch (this.grid[x][y]) {
        case Block.NO:
          break;
        case Block.NORMAL:
          this.drawBlock(Resources.images.maps[this.map].normal, x, y);
          break;
        case Block.PERMANENT:
          this.drawBlock(Resources.images.maps[this.map].permanent, x, y);
          break;

        case Block.POWER_BOMBPLUS:
          this.drawBlock(Resources.images.powers.main, x, y);
          this.drawBlock(Resources.images.powers.bombplus, x, y);
          break;
        case Block.POWER_BOMBLENGTH:
          this.drawBlock(Resources.images.powers.main, x, y);
          this.drawBlock(Resources.images.powers.bomblength, x, y);
          break;
        case Block.POWER_SPEED:
          this.drawBlock(Resources.images.powers.main, x, y);
          this.drawBlock(Resources.images.powers.speed, x, y);
          break;
        case Block.POWER_SHIELD:
          this.drawBlock(Resources.images.powers.main, x, y);
          this.drawBlock(Resources.images.powers.shield, x, y);
          break;
        case Block.POWER_KICKBOMBS:
          this.drawBlock(Resources.images.powers.main, x, y);
          this.drawBlock(Resources.images.powers.kickbombs, x, y);
          break;
        case Block.POWER_BOMBTIME:
          this.drawBlock(Resources.images.powers.main, x, y);
          this.drawBlock(Resources.images.powers.bombtime, x, y);
          break;
        case Block.POWER_SWITCHPLAYER:
          this.drawBlock(Resources.images.powers.main, x, y);
          this.drawBlock(Resources.images.powers.switchplayer, x, y);
          break;
        case Block.POWER_SICK:
          this.drawBlock(Resources.images.powers.main, x, y);
          this.drawBlock(Resources.images.powers.sick, x, y);
          break;
        case Block.POWER_BONUS:
          this.drawBlock(Resources.images.powers.main, x, y);
          this.drawBlock(Resources.images.powers.bonus, x, y);
          break;
      }
    }
  ...
  this.bombs.forEach((bomb) =>
    this.drawBlock(Resources.images.bomb, bomb.x, bomb.y, 0),
  );
