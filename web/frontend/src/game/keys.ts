  ...
  static init(g: Game) {
    ...
    if (!g.isMobile) {
      ...
      function updateKeysPressed() {
        Keys.keysPressed.up =
          Keys.keysPressed.left =
          Keys.keysPressed.down =
          Keys.keysPressed.right =
          Keys.keysPressed.bomb =
            false;
    ...
    if (g.isMobile) {
      ...
      const butLeft = document.querySelector(
        '#button-left',
      ) as HTMLButtonElement;
      const butDown = document.querySelector(
        '#button-down',
      ) as HTMLButtonElement;
      const butRight = document.querySelector(
        '#button-right',
      ) as HTMLButtonElement;
      const butBomb = document.querySelector(
        '#button-bomb',
      ) as HTMLButtonElement;
      ...
      movingButtons.forEach(({ button, action }) => {
        button.addEventListener('touchstart', () => {
          if (buttonPressed) {
            // if exists, remove the last press
            buttonPressed.dispatchEvent(new Event('touchend'));
          }

          const realAction = g.myPlayer.switchedKeys
            ? revAction[action]
            : action;
      ...
      document.addEventListener('switchkeyschange', () => {
        [Keys.keysPressed.left, Keys.keysPressed.right] = [
          Keys.keysPressed.right,
          Keys.keysPressed.left,
        ];
        [Keys.keysPressed.up, Keys.keysPressed.down] = [
          Keys.keysPressed.down,
          Keys.keysPressed.up,
        ];
      });
