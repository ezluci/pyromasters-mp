...
type PlayerAttribute = 'isOwner' | 'wins' | 'kills' | 'color' | 'coords' |
         'dead' | 'animState' | 'speed' | 'bombCount' | 'bombTime' |
         'bombLength' | 'shield' | 'sick' | 'kickBombs';

export function sendPacket_playerAttribute(this: typeof OutPackets, target: Room | WebSocket, sok: WebSocket, attribute: PlayerAttribute) {
   const packetLength = 2 + (
      attribute === 'dead' || attribute === 'animState' || attribute === 'speed' ||
      attribute === 'bombCount' || attribute === 'bombTime' || attribute === 'bombLength' ||
      attribute === 'shield' || attribute === 'sick' || attribute === 'kickBombs'
         ? 1 :
   ...
   if (packetLength === 0) {
      return console.error(`sendPacket_playerAttribute: attribute '${attribute}' unhandled`);
   }

   const packet = new Uint8Array(packetLength);
   packet[1] = (sok.color === null ? 0 : Object.values(Color).indexOf(sok.color) + 1);
   ...
   switch (attribute) {
      case 'isOwner':
         ...
         for (let i = 0; i < sok.name.length; ++i) {
            packet[2 + i] = sok.name.charCodeAt(i);
         }
         break;
      case 'wins':
         packet[0] = 1;
