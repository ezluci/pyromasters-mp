...
export function processPacket_startGame(sok: WebSocket, packet: Buffer) {
  ...
  if (
    ...
    OutPackets.send_error(
      sok,
      `tryStart: Room is in ${sok.room.status} status.`,
    );
    return;
  }
  ...
  let map =
    packet[1] >= Object.values(Map).length
      ? undefined
  ...
  if (playersAlive.length === 0) {
    OutPackets.send_error(
      sok,
      "tryStart: You can't start the game with NO PLAYERS, silly!",
    );
    return;
  }
...
function generateGrid(map: string): Block[][] {
  const grid: Block[][] = Array.from({ length: BLOCKS_HORIZONTALLY }, () =>
