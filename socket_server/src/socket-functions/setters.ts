...
export function setShield(this: WebSocket, value: boolean): void {
   if (value === false) {
      ...
      const newFuncId: number | undefined = this.room.ticks.addFunc(() => { this.setShield(false) }, SHIELD_TIME_TICKS);
...
export function setSick(this: WebSocket, value: boolean): void {
   if (value === false) {
      ...
      const newFuncId: number | undefined = this.room.ticks.addFunc(() => { this.setSick(false) }, SICK_TIME_TICKS);
      if (typeof newFuncId === 'number') {
