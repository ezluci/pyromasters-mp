...
export function isDying(this: WebSocket): false | Color[] {
  const sok = this;
  if (sok.shield) {
    return false;
  }
  ...
  if (
    deadBlk1 !== null &&
    sok.room.grid[deadBlk1.x][deadBlk1.y] === Block.PERMANENT
  ) {
    return [];
  }
  if (
    deadBlk2 !== null &&
    sok.room.grid[deadBlk2.x][deadBlk2.y] === Block.PERMANENT
  ) {
    return [];
  }
  ...
  Object.values(Color).forEach((assistColor) => {
    ...
    if (
      deadBlk1 !== null &&
      sok.room.getFlame(deadBlk1.x, deadBlk1.y, sok.room[assistColor])
    ) {
      assisted = true;
    }
    if (
      deadBlk2 !== null &&
      sok.room.getFlame(deadBlk2.x, deadBlk2.y, sok.room[assistColor])
    ) {
      assisted = true;
    }
