...
export function getBomb(
  ...
  if (arg2 !== undefined) {
    ...
    return room.bombs.find(
      (bomb) => Math.round(bomb.x) === arg1 && Math.round(bomb.y) === arg2,
    );
  } else {
    // id is arg1
...
export function getFlame(
  ...
  const room = this;
  return room.flames.find(
    (flame) => flame.x === x && flame.y === y && flame.owner === owner,
  );
}
...
export function explodeBomb(
  ...
  function explodeHelper(x: number, y: number): boolean {
    ...
    if (
      room.grid[x][y] !== Block.PERMANENT &&
      flames.filter(
        (flame) => flame.x === x && flame.y === y && flame.owner === bombOwner,
      ).length === 0
    ) {
      flames.push({
        x: x,
        y: y,
        owner: bombOwner,
        oldBlock: room.grid[x][y],
        wasBomb: false,
        tickFuncId: undefined,
      });
    }
  ...
  for (
    let yy = y + 1;
    yy <= Math.min(BLOCKS_VERTICALLY - 1, y + bombLength);
    ++yy
  ) {
    if (explodeHelper(x, yy)) {
      break;
  ...
  for (
    let xx = x + 1;
    xx <= Math.min(BLOCKS_HORIZONTALLY - 1, x + bombLength);
    ++xx
  ) {
    if (explodeHelper(xx, y)) {
      break;
...
export function removeFlame(
  ...
  if (!flame) {
    logger.error('error removeFlame');
    return;
  }
...
function breakLoop(block: Block) {
  return (
    block === Block.NORMAL || block === Block.PERMANENT || isPowerup(block)
  );
}
